...
class AuditLog(commands.Cog):
    ...
    async def on_member_update(self, before: discord.Member, after: discord.Member):
        try:
            ...
            logging.getLogger("AuditLog").error(f"Failed on_member_update: {e}")
    ...
    async def on_raw_message_delete(self, payload: RawMessageDeleteEvent):
        try:
            ...
            logging.getLogger("AuditLog").error(f"Failed on_raw_message_delete: {e}")
    ...
    async def sync_messages(self, channels):
        ...
        for channel in channels:
            ...
            if messages:
                dbMessages = await get_messages_by_channel(str(channel.id), self.client.database, start_time)
    ...
    async def sync_database(self):
        ...
        cutoff = datetime.utcnow().astimezone(timezone.utc) - timedelta(hours=1)
        valid_channels = [
        ...
        message_count = await self.sync_messages(valid_channels)
        ...
        if message_count > 0 or member_count > 0:
            logging.getLogger('AuditLog').warning(response)
